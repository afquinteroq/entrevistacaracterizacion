...
public class puntosDTAdapter extends ArrayAdapter {
    List<EMC_DTPUNTOSATENCION> items;

    public puntosDTAdapter(Context context, int tvID, List<EMC_DTPUNTOSATENCION> objects){
    ...
    public View getCustomView(int position, View convertView, ViewGroup parent)
    {
        ...
        valID.setText(items.get(position).getIdpuntoatencion().toString());
        TextView valNombre=(TextView)row.findViewById(R.id.valNombre);
        valNombre.setText(items.get(position).getPuntoatencion());
        return row;
    }
